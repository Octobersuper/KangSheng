...
public class MahjongUtils {
    ...
    public void getBrandKe(RoomBean roomBean, UserBean userBean, Integer brand, List<Integer> tingList,int huType) {
        // 卡张 +1 胡牌时胡的是三张序数牌的中间一张，并且只能胡这一张牌。例如234中的3。
        if(huType==0){
            userBean.getBrands().add(brand);
        }

        if (kz(userBean, brand, tingList)) {
            userBean.setPower(1);
            System.err.println("卡张+1");
            userBean.getRecordMsgList().add("卡张+1");
        }
        // 边张 +1 胡牌时胡的789中的7或者123中的3，并且只能胡这一张牌。
        if (bz(userBean, brand, tingList)) {
            userBean.setPower(1);
            System.err.println("边张+1");
            userBean.getRecordMsgList().add("边张+1");
            removeList(userBean,"卡张+1",-1);
        }

        // 缺一门 +1
        if (qym(roomBean, userBean)) {
            userBean.setPower(1);
            System.err.println("缺一门+1");
            userBean.getRecordMsgList().add("缺一门+1");
            userBean.setQ1(1);
        }
        // 大对子 +5指四副搭子都是三张（或四张）相同的任意牌，并且只有两门花色。
        if (ddz(roomBean, userBean,brand)) {
            userBean.setPower(5);
            System.err.println("大对子+5");
            userBean.getRecordMsgList().add("大对子+5");
        }
        // 卡星五
        if (kwx(userBean, brand, tingList)) {
            userBean.setPower(1);
            System.err.println("卡五星+1");
            userBean.getRecordMsgList().add("卡五星+1");
        }

        // 一条龙
        if (ytl(userBean)) {
            userBean.setPower(5);
            System.err.println("一条龙+5");
            userBean.getRecordMsgList().add("一条龙+5");
        }
        // 清一色（混清）
        if (qysh(userBean)) {
            userBean.setPower(5);
            System.err.println("清一色（混）+5");
            userBean.getRecordMsgList().add("清一色（混）+5");
            userBean.setPower(2);
            System.err.println("缺两门+2");
            userBean.getRecordMsgList().add("缺两门+2");
            removeList(userBean,"缺一门+1",-1);
        }
        // 清一色（纯清）
        if (qysc(userBean)) {
            userBean.setPower(10);
            System.err.println("清一色（纯）+10");
            userBean.getRecordMsgList().add("清一色（纯）+10");
            userBean.setPower(2);
            System.err.println("缺两门+2");
            userBean.getRecordMsgList().add("缺两门+2");
            removeList(userBean,"缺一门+1",-1);
        ...
        // 四归一
        sgy(userBean, brand,huType);
        //卡张不能有单吊
        if(!userBean.getRecordMsgList().contains("卡张+1") && !userBean.getRecordMsgList().contains("边张+1")){
            // 单钓 +1 胡牌时胡的是将牌，并且只能胡这一张牌。
            if (dd(userBean, brand, tingList)) {
                userBean.setPower(1);
                System.err.println("单钓+1");
                userBean.getRecordMsgList().add("单钓+1");

            }
        }

        //金钩钓
        if (jgd(roomBean, userBean, brand)) {
            userBean.setPower(5);
            System.err.println("金钩钓+5");
            userBean.getRecordMsgList().add("金钩钓+5");
            removeList(userBean,"单钓+1",-1);
            userBean.setPower(1);
            userBean.getRecordMsgList().add("单钓+1");
        }
        ...
        if (lqd(userBean, brand)) {
            userBean.setPower(10);
            System.err.println("龙七对+10");
            userBean.getRecordMsgList().add("龙七对+10");
            removeList(userBean,"单钓+1",-1);
            userBean.setPower(1);
            userBean.getRecordMsgList().add("单钓+1");
            //removeList(userBean,"四归一+1",-1);
            removeList(userBean,"卡张+1",-1);
            removeList(userBean,"边张+1",-1);
        } else
            // 暗七对
            if (aqd(userBean,brand)) {
                userBean.setPower(5);
                System.err.println("暗七对+5");
                userBean.getRecordMsgList().add("暗七对+5");
                removeList(userBean,"单钓+1",-1);
                userBean.setPower(1);
                userBean.getRecordMsgList().add("单钓+1");
                removeList(userBean,"卡张+1",-1);
        ...
        if (xsyh(userBean)) {
            userBean.setPower(5);
            System.err.println("小三元汇+5");
            userBean.getRecordMsgList().add("小三元汇+5");
        }
        // 大三元汇
        if (dsyh(userBean)) {
            userBean.setPower(5);
            System.err.println("大三元汇+5*2");
            userBean.getRecordMsgList().add("大三元汇+5*2");
        }
        // 十八罗汉
        if (sblh(userBean)) {
            userBean.setPower(108);
            System.err.println("十八罗汉+108");
            userBean.getRecordMsgList().add("十八罗汉+108");
        }
        // 3红+3白=1
        if (threehb(userBean) && (!userBean.getRecordMsgList().contains("大飞机+7")&&!userBean.getRecordMsgList().contains("大飞机+14")&&!userBean.getRecordMsgList().contains("大飞机+21"))) {
            userBean.setPower(1);
            System.err.println("小飞机+1");
            userBean.getRecordMsgList().add("小飞机+1");
        }
        // 3白+3发=1
        /*if (threebf(userBean)) {
            userBean.setPower(1);
            System.err.println("3白+3发+1");
            userBean.getRecordMsgList().add("3白+3发+1");
        }*/
        // 3发+3红=1
       /* if (threefh(userBean)) {
            userBean.setPower(1);
            System.err.println("3发+3红+1");
            userBean.getRecordMsgList().add("3发+3红+1");
        }*/
        // 4红+4白=火箭（7颗）
        fourhb(userBean);
        // 4白+4发=火箭（7颗）
       /* if (fourbf(userBean)) {
            userBean.setPower(7);
            System.err.println("4白+4发+7");
            userBean.getRecordMsgList().add("4白+4发+7");
        }*/
        // 4发+4红=火箭（7颗）
        /*if (fourfh(userBean)) {
            userBean.setPower(7);
            System.err.println("4发+4红+7");
            userBean.getRecordMsgList().add("4发+4红+7");
        }*/
        // 门清 +1颗
        if (mq(roomBean, userBean)) {
            userBean.setPower(1);
            System.err.println("门清+1");
            userBean.getRecordMsgList().add("门清+1");
            // 大牌门清
            if (dpmq(roomBean, userBean)) {
                userBean.setPower(5);
                System.err.println("大牌门请+5");
                userBean.getRecordMsgList().add("大牌门请+5");
            }
        }
        // 平胡 +1颗
        if (ph(roomBean, userBean)) {
            System.err.println("平胡");
            userBean.getRecordMsgList().add("平胡");
        }
        if(userBean.getRecordMsgList().contains("大三元汇+5*2")){
            userBean.setPower(userBean.getPower_number());
        }
    }
    ...
    private boolean dpmq(RoomBean roomBean, UserBean userBean) {
        ...
        for (String s : recordList) {
            if (s.contains("大对子") || s.contains("一条龙") || s.contains("龙七对") || s.contains("暗七对") || s.contains("清一色（纯）") || s.contains("清一色（混）")) {
                return true;
            }
        }
        return false;
    ...
    private boolean ph(RoomBean roomBean, UserBean userBean) {
        List<String> recordList = userBean.getRecordMsgList();
        for (String s : recordList) {
            if (s.contains("大对子") || s.contains("一条龙") || s.contains("龙七对") || s.contains("暗七对") || s.contains("清一色（纯）") || s.contains("清一色（混）")|| s.contains("大三元")|| s.contains("大飞机")|| s.contains("小飞机")) {
                return false;
            }
        }
        return true;
    ...
    private boolean qym(RoomBean roomBean, UserBean userBean) {
        if (roomBean.getAssured_brand() == 1 || roomBean.getMax_person()==2) {
            return true;
        }
        return false;
    }
    ...
    private boolean dd(UserBean userBean, int brand, List<Integer> tingList) {
        ...
        if (count == 2) {
            for (Integer i : tingList) {
                if (i == brand_value && tingList.size() == 1) {
                    //单吊情况下 判断是否卡五星
                    if (brand_value == 4 || brand_value == 13 || brand_value == 22) {
                        removeList(userBean,"卡五星+1",-1);
                        userBean.setPower(1);
                        System.err.println("卡五星+1");
                        userBean.getRecordMsgList().add("卡五星+1");
                    }
