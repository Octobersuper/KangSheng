...
import com.zcf.mahjong.service.Exit_Game;
...
public class RoomBean {
	...
	private int dice;
	// 换三张
	private int adjust_brand;
	// 相同花色
	private int same_colour;
	// 定缺
	private int assured_brand;
	// 字牌算飞机
	private int aircraft;
	// 测距功能
	private int ranging;
	// 主动测距
	private int initiative_ranging;
	//0自摸 1点炮
	private int hutype;
	//检测胡人数
	private int flowNum;
	...
	public RoomBean() {
		//创建倒计时解散房间线程类
		//this.exit_time = new Exit_Game(null,this);
		// 用户座位
	...
	public int getNextUserId() {
		...
		do{
			...
			// 为此用户创建一个倒计时线程
			end_userid = userid;
            //this.getExit_time().updateUser(this.getUserBean(userid));
	...
	public void getRoomBean_Custom(String table, Map<String, Object> returnMap) {
		...
		for (String name : names) {
			...
			if ("adjust_brand".equals(name))
				returnMap.put("adjust_brand", this.adjust_brand);
			if ("same_colour".equals(name))
				returnMap.put("same_colour", this.same_colour);
			if ("assured_brand".equals(name))
				returnMap.put("assured_brand", this.assured_brand);
			if ("aircraft".equals(name))
				returnMap.put("aircraft", this.aircraft);
			if ("ranging".equals(name))
				returnMap.put("ranging", this.ranging);
			if ("initiative_ranging".equals(name))
				returnMap.put("initiative_ranging", this.initiative_ranging);
			if ("data".equals(name))
	...
	public int getAdjust_brand() {
		return adjust_brand;
	}

	public void setAdjust_brand(int adjust_brand) {
		this.adjust_brand = adjust_brand;
	}

	public int getSame_colour() {
		return same_colour;
	}

	public void setSame_colour(int same_colour) {
		this.same_colour = same_colour;
	}

	public int getAssured_brand() {
		return assured_brand;
	}

	public void setAssured_brand(int assured_brand) {
		this.assured_brand = assured_brand;
	}
	...
	public int getInitiative_ranging() {
		return initiative_ranging;
	}

	public void setInitiative_ranging(int initiative_ranging) {
		this.initiative_ranging = initiative_ranging;
	}
